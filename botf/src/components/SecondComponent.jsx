...
//           );
//         }
//         console.log(images, "444444444444444444444")
//         console.log(uploadedImages, "bbbbbbbbbbbbbbbbbbbbbb")
//         console.log("Images sent as a grid to Telegram successfully!");
//       };

//       // Step 2: Upload grid images
//       const images = secondFormData.images;
//       await uploadImagesToTelegram(images, TELEGRAM_CHAT_ID, TELEGRAM_BOT_TOKEN);
...
//       };
//       const amenitiesFormatted = formatAmenitiesInTwoColumns(secondFormData.amenities);
...
// 🐕 Pets: ${secondFormData.additional === "PetsRestriction" ? "#Allowed" : "#NotAllowed"}
// ⏰ #${secondFormData?.term==="Long-term"? "Longterm": ""} #${secondFormData?.termDuration ==="1 month"?"1month" :secondFormData?.termDuration ==="6 months"? "6month":secondFormData?.termDuration ==="12 months"? "12month": ""  }
...
//       // Embed the text in the Telegram URL
...
//                 key={type}
//                 onClick={() => setSecondFormData({ ...secondFormData, dealType: type })}
//                 className={`px-4 py-2 rounded-md ${secondFormData.dealType === type
//                     ? 'bg-blue-600 text-white'
//                     : 'bg-gray-200 text-gray-600'
//                   }`}
//               >
...
//               <div className="flex gap-4">
//                 <button
//                   className={`flex-1 px-4 py-2 rounded-md ${secondFormData.termDuration === '1 month'
//                       ? 'bg-blue-600 text-white'
//                       : 'bg-gray-200 text-gray-600'
//                     }`}
//                   onClick={() =>
...
//                 </button>
//                 <button
//                   className={`flex-1 px-4 py-2 rounded-md ${secondFormData.termDuration === '6 months'
//                       ? 'bg-blue-600 text-white'
//                       : 'bg-gray-200 text-gray-600'
//                     }`}
//                   onClick={() =>
...
//                 </button>
//                 <button
//                   className={`flex-1 px-4 py-2 rounded-md ${secondFormData.termDuration === '12 months'
//                       ? 'bg-blue-600 text-white'
//                       : 'bg-gray-200 text-gray-600'
//                     }`}
//                   onClick={() =>
...
//                     value={secondFormData.commission}
//                     onChange={(e) =>
//                       setSecondFormData({ ...secondFormData, commission: Number(e.target.value) })
//                     }
//                     className="w-full p-2 border border-gray-300 rounded-md"
//                   />
...
//                     }))
//                   }
//                   className={`px-4 py-2 rounded-md ${secondFormData.amenities.includes(option)
//                       ? 'bg-blue-600 text-white'
//                       : 'bg-gray-200 text-gray-600'
//                     }`}
//                 >
...
const SecondComponent = ({ setStep }) => {
  const [secondFormData, setSecondFormData] = useState({
    ...
    totalFloors: 0,
    termDuration: "",
    address: JSON.parse(localStorage.getItem("form1"))?.address || "helo",
  ...
  const handlePublish = async () => {
    ...
    try {
      const TELEGRAM_BOT_TOKEN =
        "7712916176:AAF15UqOplv1hTdJVxILWoUOEefEKjGJOso";
      const TELEGRAM_CHAT_ID = "-4545005015,";
      ...
      const uploadImagesToTelegram = async (images, chatId, botToken) => {
        try {
          ...
          // Step 1: Upload images and collect file_id and message_id
          for (const photo of images) {
            const formData = new FormData();
            formData.append("chat_id", chatId); // Specify the recipient's chat_id
            formData.append("photo", await fetch(photo).then((res) => res.blob()));
      
            // Upload the photo to Telegram for the specific user
            const response = await axios.post(
             ` https://api.telegram.org/bot${botToken}/sendPhoto`,
              formData
            );
            ...
            if (fileId && messageId) {
              ...
              messageIdsToDelete.push(messageId); // Store message_id for deletion
            } else {
              throw new Error("Failed to retrieve file_id or message_id");
            }
          }
      
          // Step 2: Send all images as a grid
          if (uploadedImages.length > 0) {
            await axios.post(
             ` https://api.telegram.org/bot${botToken}/sendMediaGroup`,
              {
          ...
          // Step 3: Delete the individual image messages after they have been grouped
          ...
          console.log("Individual image messages deleted successfully!");
        } catch (error) {
          console.error("Error uploading or deleting images to Telegram:", error.response?.data || error.message);
        }
      };
      ...
      // Step 3: Send the text message
      let message = `
      #${secondFormData?.city}  #${secondFormData?.district} 🏢#${
        secondFormData?.metro
      } 
📍${secondFormData.address}
...
🏠 ${secondFormData.area} Sq.m | #${secondFormData?.floor}floor | #${
        secondFormData?.bathrooms
      }Bath
...
⏰ #${secondFormData?.term === "Long-term" ? "Longterm" : ""} #${
        secondFormData?.termDuration === "1 month"
          ? "1month"
          : secondFormData?.termDuration === "6 months"
          ? "6month"
...
💳 #${secondFormData?.paymentMethod}   
💰 ${secondFormData.price}${
        secondFormData.currency == "USD" ? "$" : "₾"
      } | Deposit ${secondFormData.price}${
        secondFormData.currency == "USD" ? "$" : "₾"
      }
 0% Commission
 #Price${secondFormData.price}
            ...
            <div className="space-y-4 mt-4">
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full p-2 border border-gray-300 rounded-md"
                >
                  <option value="First and last month">
